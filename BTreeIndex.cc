...
RC BTreeIndex::insert(int key, const RecordId& rid)
{
	...
	if (rootPid != -1)
	{
	    ...
	    l.read(pid, pf);
		//if(key == 491)
			//printf("leafcount is %d", l.getKeyCount());
	}
	else
	{
		pid = pf.endPid();
	...
	{
		//printf("about to insert key: %d \n", key);
		l.insert(key, rid);
		//printf("\nafter insertion\n");
		int preid;
		l.locate(100000, preid);
		//printf("\n");
		l.write(pid, pf);
	}
	else
	{
		...
		l.write(pid, pf);
			int pprid;
			//printf("sibling\n");
			sibling.locate(100000, pprid);
			//printf("leaf\n");
			l.locate(100000, pprid);
...
RC BTreeIndex::insert_into_parent(int level, PageId childpid, int key, PageId sib_pid)
{
	...
	if(parent.getKeyCount() < MAX_NODE_SIZE)
	{
		...
		//printf("parent node \n");
		int prid;
		parent.locateChildPtr(1000000000, prid);
	}
	else
	{
		...
		int prid;
		parent.locateChildPtr(1000000000, prid);
		//printf("sibling parent \n");
		sibling.locateChildPtr(10000000000, prid);
...
{
	//fprintf(stdout, "looking for key=%d\n", searchKey);
	if(rootPid == -1)
	...
	//printf("eid i got in locate: %d with rc = %d and pid = %d\n", eid, rc, pid);
	cursor.eid = eid;
	cursor.pid = pid;
...
RC BTreeIndex::readForward(IndexCursor& cursor, int& key, RecordId& rid)
{
	...
	//l.read(cursor.pid, pf);
	 //fprintf(stdout,"setting ic to eid=%d, pid=%d, key=%d, count = %d\n", cursor.eid, cursor.pid, key, l.getKeyCount());
	return code;
}
