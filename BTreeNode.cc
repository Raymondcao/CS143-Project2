                              ...
                              BTLeafNode& sibling, int& siblingKey)
{
	...
	if (eid<leftSize){
		while(count> leftSize-1){
			count--;
			memcpy(&curKey, buffer+sizeof(count)+ENTRY_SIZE*count, sizeof(curKey));
			memcpy(&curRid, buffer+sizeof(count)+sizeof(curKey)+ENTRY_SIZE*count, sizeof(curRid));
...
RC BTLeafNode::locate(int searchKey, int& eid)
{
	...
	for(int i=0; i<count; i++){
		...
		//printf("key: %d with i=%d ", curKey, i);
		if (curKey==searchKey){
			eid = i;
			//printf("\n");
			return 0;
